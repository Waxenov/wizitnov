        ...
        <div class="desktop:w-full max-w-full tablet:px-6 minimal:px-2">
            ...
            <div class="flex mx-auto mb-10 items-end px-0">
                <div class="flex w-full h-fit">
                    <form action="{{ route('trash') }}" method="GET" class="w-full">
                                ...
                                @foreach($userOrders as $order)
                                    ...
                                    <tr class="bg-oldpaper hover:bg-brownpaper/30 font-base text-center py-6 border-x-4 border-x-coffee border-b-4 border-b-coffee">
                                        ...
                                        <td class="bg-milano border-l-4 border-l-coffee items-center">
                                            <form method="POST" action="{{ route('trash.processing', $order->id) }}">
                                                @csrf
                                                <button type="submit" class="inline-flex py-1 items-center font-big text-xl text-hipnymph tracking-widest hover:text-brownpaper bg-milano">оживить</button>
                                            </form>
                                            <form action="{{ route('trash.destroy', $order->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex py-1 items-center font-big text-xl text-hipnymph tracking-widest hover:text-brownpaper bg-milano">удалить</button>
                                            </form>
                                        </td>
                                    </tr>
                                    @endif
            ...
            @if(Auth::user()->status == 'active')
                ...
                @else
                    <div class="w-full flex flex-col justify-center items-center py-4">
                        <h1 class="text-coffee text-2xl font-pix font-thin text-center">
                            {{ __('Оживление - возвращение заказа со статусом "ожидание"') }}
                        </h1>
                        <h1 class="text-coffee text-2xl font-pix font-thin text-center">
                            {{ __('Удаление - заказ удаляется по настоящему') }}
                        </h1>
                    </div>
                @endif
            @endif
                    ...
                    @foreach ($userOrders as $order)
                        <div id="cardorder" class="flex w-full flex-col justify-center items-center mb-4 border-4 border-coffee bg-oldpaper">
                            <div class="cardhead w-full items-center py-2 px-2">
                                <h1 class="text-coffee text-2xl font-pix">Заказ #{{ $order->id }}</h1>
                                ...
                                <div id="cardorderbg" class="bg-brownpaper/30 px-4 border-t-4 border-t-coffee">
                                    ...
                                    <ul>
                                        ...
                                        <li class="flex flex-col justify-between font-base text-start py-2">
                                            ...
                                            <h2 class="text-coffee font-thin font-base text-start text-xl"> {{ $order->weight }} </h2>
                                        </li>
                                        <li class="flex flex-col justify-between font-base text-start py-2">
                                        ...
                                        <li class="flex flex-col justify-between font-base text-start py-2">
                                            <h1 class="text-coffee font-thin font-big text-xl"> Удалить </h1>
